...
########### Gain d'information #########
X_select= SelectKBest(score_func= mutual_info_classif, k=7)
fit = X_select.fit(abs(X), y)

list_features_Gain= fit.scores_

high_score_features_Gain = []
list_gain = []
for score, f_name in sorted(zip(list_features_Gain, X.columns), reverse=True)[:7]:
        list_gain.append(f_name)
        high_score_features_Gain.append([f_name, score])

Table_Features_Gain = tabulate(high_score_features_Gain , headers=['Features', 'Score'], tablefmt="grid", showindex="always")
print(Table_Features_Gain)
X = legitime_polluters[list_gain] # Features
y = legitime_polluters.Class # Target variable
...
print('F-Measure: %.3f' % score_Adaboost)# calculate AUC
