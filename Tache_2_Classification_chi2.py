...
#####le test Chi-2 (Chi Squared)
x_select_Chi_2= SelectKBest(score_func= chi2, k=7)
fit_chi_2 = x_select_Chi_2.fit(abs(X), y)

list_features_Chi_2= fit_chi_2.scores_

high_score_features_chi_2 = []
for score, f_name in sorted(zip(list_features_Chi_2, X.columns), reverse=True)[:7]:
        high_score_features_chi_2.append([f_name, score])

# print(tabulate(high_score_features_chi_2 , headers=['Features', 'Score'], tablefmt="grid", showindex="always"))
Table_Features_chi2=tabulate(high_score_features_chi_2 , headers=['Features', 'Score'], tablefmt="grid", showindex="always")
print(Table_Features_chi2)
def Extract(lst):
    return [item[0] for item in lst]
features_name= Extract(high_score_features_chi_2)
### New train-test ###
X = legitime_polluters[features_name] # Features
y = legitime_polluters.Class # Target variable
X_train1, X_test1, y_train1, y_test1 = train_test_split(X, y, test_size=0.2, random_state=1) # 80% training and 20% test

########################################DecisionTree###############################################
clf_DecisionTree = DecisionTreeClassifier()  # Create Decision Tree classifer object
# Train Decision Tree Classifer
clf_DecisionTree = clf_DecisionTree.fit(X_train1,y_train1)
#Predict the response for test dataset
y_pred_DecisionTree = clf_DecisionTree.predict(X_test1)
# Model Accuracy, how often is the classifier correct?
acc_DecisionTree=metrics.accuracy_score(y_test1, y_pred_DecisionTree)
print("Accuracy:", acc_DecisionTree )
### confusion_matrix
x  = confusion_matrix(y_test, y_pred_DecisionTree, labels= [1,0])
print(x)
...
auc_DecisionTree = roc_auc_score(y_test1, y_pred_DecisionTree)
...
# Train Classifer
clf_RandomForest.fit(X_train1,y_train1)
#Predict the response for test dataset
y_pred_Random_Forest=clf_RandomForest.predict(X_test1)
## calculate the accuracy
acc_RandomForest= metrics.accuracy_score(y_test1, y_pred_Random_Forest)
...
###3 calculate AUC
auc_RandomForest = roc_auc_score(y_test1, y_pred_Random_Forest)
...
# Train Classifer
clf_Bagging.fit(X_train1, y_train1)
#Predict the response for test dataset
y_pred_Bagging=clf_Bagging.predict(X_test1)
## calculate the accuracy
acc_Bagging = metrics.accuracy_score(y_test1, y_pred_Bagging)
...
###3 calculate AUC
auc_Bagging = roc_auc_score(y_test1, y_pred_Bagging)
...
# Train Classifer
clf_Adaboost= clf_Adaboost.fit(X_train1, y_train1)
#Predict the response for test dataset
y_pred_Boosting = clf_Adaboost.predict(X_test1)
## calculate the accuracy
acc_Adaboost =metrics.accuracy_score(y_test1, y_pred_Boosting)
...
# calculate AUC
auc_Adaboost = roc_auc_score(y_test1, y_pred_Boosting)
...
# Train Classifer
clf_Gaussian= clf_Gaussian.fit(X_train1, y_train1)
#Predict the response for test dataset
y_pred_Gaussian = clf_Gaussian.predict(X_test1)
## calculate the accuracy
acc_Gaussian = metrics.accuracy_score(y_test1, y_pred_Gaussian)
...
print('F-Measure: %.3f' % score_Adaboost)
###3 calculate AUC
auc_Adaboost = roc_auc_score(y_test1, y_pred_Gaussian)
print('AUC: %.3f' % auc_Adaboost)
